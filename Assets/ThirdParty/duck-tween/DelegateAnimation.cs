...
namespace DUCK.Tween
{
    ...
    public sealed class DelegateAnimation<T> : DelegateAnimation where T : AbstractAnimation
    {
        ...
        public override void PlayWhile(Action onComplete, Func<bool> updateWhile, Action onAbort = null)
        {
            base.PlayWhile(onComplete, updateWhile, onAbort);

            if (CreateDelegateAnimation())
            {
                Animation.PlayWhile(NotifyAnimationComplete, onUpdateWhileCallback, base.Abort);
            }
            else
            {
                base.Abort();
