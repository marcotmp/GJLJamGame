...
namespace DUCK.Tween.Extensions
{
    public static partial class TransformAnimationExtensions
    {
        ...
        public static DelegateAnimation<ScaleAnimationWhile> ScaleToWhile(this Transform transform, float to, Func<bool> scaleWhile, float duration = 1f, Func<float, float> easingFunction = null)
        {
            var animation = new DelegateAnimation<ScaleAnimationWhile>(() =>
                new ScaleAnimationWhile(transform.gameObject, transform.localScale, Vector3.one * to, scaleWhile, duration, easingFunction));
            return animation;
        }
